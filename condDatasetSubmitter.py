    ...
    if Type == 'HLT':
        ...
        if options.HLT:
            HLTBase.update({"steps":"L1REPACK:Full,HLT:%s" % (options.HLT),
                            #"custcommands":"\ntry:\n\tif process.RatesMonitoring in process.schedule: process.schedule.remove( process.RatesMonitoring );\nexcept: pass",
                            #"custcommands":"\ntry:\n\tif process.L1RePack_step in process.schedule: process.schedule.insert( 0, process.L1RePack_step );\nexcept: pass",
                            "custcommands":"\ntry:\n\tif process.unpackGt_step not in process.schedule: process.schedule.insert( 0, process.unpackGt_step );\nexcept: pass",
                            "custconditions":"",
...
def createCMSSWConfigs(options,confCondDictionary,allRunsAndBlocks):
    ...
    for c in confCondList:
        ...
        #execme(cmssw_command + '; ' + driver_command + '; ' + upload_command) ##REF and NEWCO
        base = None
